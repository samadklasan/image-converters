...
import { JpgToPngToolCardIcon } from "@/components/tools/jpg-to-png/jpg-to-png-card-icon";
...
function isValidJpgFile(file: File) {
  return isJpgToPngMimeType(file.type) || isJpgFileName(file.name);
}
...
export function JpgToPngConverter() {
  ...
  const getConversionWorker = () => {
    ...
    const nextWorker = new Worker(
      new URL("./jpg-to-png-worker.ts", import.meta.url),
      { type: "module" },
    );
    ...
    nextWorker.onmessage = (event: MessageEvent<ConversionWorkerResponse>) => {
      ...
      if (event.data.type === "success") {
        pendingRequest.resolve(
          new Blob([event.data.buffer], { type: "image/png" }),
        );
        return;
      }
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > jpgToPngMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${jpgToPngMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getJpgToPngDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this JPG to PNG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <JpgToPngToolCardIcon disableGroupHover />
              </div>
