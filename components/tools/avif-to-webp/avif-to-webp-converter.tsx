...
import { AvifToWebpToolCardIcon } from "@/components/tools/avif-to-webp/avif-to-webp-card-icon";
...
function isValidAvifFile(file: File) {
  return isAvifToWebpMimeType(file.type) || isAvifFileName(file.name);
}
...
function getZipDownloadName() {
  return "avif-to-webp-images.zip";
}
...
export function AvifToWebpConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > avifToWebpMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${avifToWebpMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getAvifToWebpDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/avif-to-webp/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this AVIF to WEBP batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <AvifToWebpToolCardIcon disableGroupHover />
              </div>
