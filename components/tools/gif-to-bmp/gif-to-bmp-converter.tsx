...
import { GifToBmpToolCardIcon } from "@/components/tools/gif-to-bmp/gif-to-bmp-card-icon";
...
function isValidGifFile(file: File) {
  return isGifToBmpMimeType(file.type) || isGifFileName(file.name);
}
...
export function GifToBmpConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > gifToBmpMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${gifToBmpMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getGifToBmpDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/gif-to-bmp/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this GIF to BMP batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <GifToBmpToolCardIcon disableGroupHover />
              </div>
