...
import { PngToTiffToolCardIcon } from "@/components/tools/png-to-tiff/png-to-tiff-card-icon";
import { createRuntimeId } from "@/lib/runtime-id";

const pngToTiffAcceptedMimeTypes = ["image/png"] as const;
const pngToTiffMaxUploadBytes = 50 * 1024 * 1024;
const pngToTiffMaxUploadMegabytes = Math.floor(
  pngToTiffMaxUploadBytes / (1024 * 1024),
);
const pngToTiffMaxInputPixels = 40_000_000;

function isPngToTiffMimeType(value: string) {
  return pngToTiffAcceptedMimeTypes.includes(
    value as (typeof pngToTiffAcceptedMimeTypes)[number],
  );
}
...
function getPngToTiffDownloadName(value: string) {
...
function isValidPngFile(file: File) {
  return isPngToTiffMimeType(file.type) || isPngFileName(file.name);
}
...
export function PngToTiffConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > pngToTiffMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${pngToTiffMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getPngToTiffDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/png-to-tiff/", {
        body: formData,
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(
          await readApiErrorMessage(
            response,
            "Unable to convert this PNG image to TIFF right now.",
          ),
        );
      }
      ...
      if (convertedBlob.size <= 0) {
        throw new Error("The converted TIFF file was empty.");
      }
      ...
      throw error instanceof Error
        ? error
        : new Error("Unable to convert this PNG image to TIFF right now.");
    } finally {
    ...
    try {
      for (const itemId of itemIdsToConvert) {
        ...
        try {
          ...
          const tiffBlob = await convertItem(latestItem);
          const nextResultUrl = registerObjectUrl(tiffBlob);
      ...
      setError("Unable to finish this PNG to TIFF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <PngToTiffToolCardIcon disableGroupHover />
              </div>
            ...
            <div className="flex flex-col items-center justify-center gap-3 sm:flex-row sm:flex-wrap">
              <div className="group relative inline-flex">
                <button
                  ...
                  className="btn-primary h-9 min-w-44 gap-2 px-5"
                >
                  {isConverting ? "Converting..." : "Convert to TIFF"}
                </button>
