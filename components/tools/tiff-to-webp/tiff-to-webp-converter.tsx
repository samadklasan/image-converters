...
import { TiffToWebpToolCardIcon } from "@/components/tools/tiff-to-webp/tiff-to-webp-card-icon";
import { createRuntimeId } from "@/lib/runtime-id";

const tiffToWebpAcceptedMimeTypes = ["image/tiff"] as const;
const tiffToWebpMaxUploadBytes = 50 * 1024 * 1024;
const tiffToWebpMaxUploadMegabytes = Math.floor(
  tiffToWebpMaxUploadBytes / (1024 * 1024),
);
const tiffToWebpMaxInputPixels = 60_000_000;
function isTiffToWebpMimeType(value: string) {
  return tiffToWebpAcceptedMimeTypes.includes(
    value as (typeof tiffToWebpAcceptedMimeTypes)[number],
  );
}
...
function getTiffToWebpDownloadName(value: string) {
...
const batchLimit = 20;
const queueLimitMessage = `You can convert up to ${batchLimit} TIFF images at a time.`;
...
function isValidTiffFile(file: File) {
  return isTiffToWebpMimeType(file.type) || isTiffFileName(file.name);
}
...
function getZipDownloadName() {
  return "tiff-to-webp-images.zip";
}
...
export function TiffToWebpConverter() {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        if (!isValidTiffFile(file)) {
          nextErrors.push(`"${file.name}" is not a valid TIFF image.`);
          continue;
        }

        if (file.size > tiffToWebpMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${tiffToWebpMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getTiffToWebpDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/tiff-to-webp/", {
        body: formData,
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(
          await readApiErrorMessage(
            response,
            "Unable to convert this TIFF image right now.",
          ),
        );
      }
      ...
      if (convertedBlob.size <= 0) {
        throw new Error("The converted WEBP file was empty.");
      }
      ...
      throw conversionError instanceof Error
        ? conversionError
        : new Error("Unable to convert this TIFF image right now.");
    } finally {
  ...
  const convertQueue = async () => {
    ...
    if (itemIdsToConvert.length === 0) {
      setError("Select at least one TIFF image first.");
      return;
    }
    ...
    try {
      ...
      setError("Unable to finish this TIFF to WEBP batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
            ...
            <div className="ui-alert ui-alert-error font-body text-sm">
              ...
              <div className="mt-2 border-t border-red-300/80 pt-2">
                <p>
                  ...
                  you have more than 20 TIFF files, save this batch first, click
