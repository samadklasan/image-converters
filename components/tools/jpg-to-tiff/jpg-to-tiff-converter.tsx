...
import { JpgToTiffToolCardIcon } from "@/components/tools/jpg-to-tiff/jpg-to-tiff-card-icon";
import { createRuntimeId } from "@/lib/runtime-id";

const jpgToTiffAcceptedMimeTypes = ["image/jpeg", "image/jpg"] as const;
const jpgToTiffMaxUploadBytes = 50 * 1024 * 1024;
const jpgToTiffMaxUploadMegabytes = Math.floor(
  jpgToTiffMaxUploadBytes / (1024 * 1024),
);
const jpgToTiffMaxInputPixels = 60_000_000;

function isJpgToTiffMimeType(value: string) {
  return jpgToTiffAcceptedMimeTypes.includes(
    value as (typeof jpgToTiffAcceptedMimeTypes)[number],
  );
}
...
function getJpgToTiffDownloadName(value: string) {
...
function isValidJpgFile(file: File) {
  return isJpgToTiffMimeType(file.type) || isJpgFileName(file.name);
}
...
export function JpgToTiffConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > jpgToTiffMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${jpgToTiffMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getJpgToTiffDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/jpg-to-tiff/", {
        body: formData,
        method: "POST",
      ...
      throw error instanceof Error
        ? error
        : new Error("Unable to convert this JPG image right now.");
    } finally {
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this JPG to TIFF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <JpgToTiffToolCardIcon disableGroupHover />
              </div>
              ...
              <div className="max-w-2xl space-y-2">
                ...
                <div className="space-y-1 font-body text-sm leading-6 text-black/65 sm:text-[14px]">
                  <p>Files are used only to complete the conversion request.</p>
