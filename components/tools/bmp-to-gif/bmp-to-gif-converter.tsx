...
import { BmpToGifToolCardIcon } from "@/components/tools/bmp-to-gif/bmp-to-gif-card-icon";
...
function isValidBmpFile(file: File) {
  return isBmpToGifMimeType(file.type) || isBmpFileName(file.name);
}
...
export function BmpToGifConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > bmpToGifMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${bmpToGifMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getBmpToGifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/bmp-to-gif/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this BMP to GIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <BmpToGifToolCardIcon disableGroupHover />
              </div>
