...
import { JpgToAvifToolCardIcon } from "@/components/tools/jpg-to-avif/jpg-to-avif-card-icon";
...
function isValidJpgFile(file: File) {
  return isJpgToAvifMimeType(file.type) || isJpgFileName(file.name);
}
...
export function JpgToAvifConverter() {
  ...
  const getConversionWorker = () => {
    ...
    const nextWorker = new Worker(
      new URL("./jpg-to-avif-worker.ts", import.meta.url),
      { type: "module" },
    );
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > jpgToAvifMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${jpgToAvifMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getJpgToAvifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this JPG to AVIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <JpgToAvifToolCardIcon disableGroupHover />
              </div>
