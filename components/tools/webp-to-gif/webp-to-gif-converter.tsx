...
import { WebpToGifToolCardIcon } from "@/components/tools/webp-to-gif/webp-to-gif-card-icon";
...
function isValidWebpFile(file: File) {
  return isWebpToGifMimeType(file.type) || isWebpFileName(file.name);
}
...
export function WebpToGifConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > webpToGifMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${webpToGifMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getWebpToGifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/webp-to-gif/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this WEBP to GIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <WebpToGifToolCardIcon disableGroupHover />
              </div>
