...
import { WebpToTiffToolCardIcon } from "@/components/tools/webp-to-tiff/webp-to-tiff-card-icon";
...
function isValidWebpFile(file: File) {
  return isWebpToTiffMimeType(file.type) || isWebpFileName(file.name);
}
...
function getZipDownloadName() {
  return "webp-to-tiff-images.zip";
}
...
export function WebpToTiffConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > webpToTiffMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${webpToTiffMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getWebpToTiffDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/webp-to-tiff/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this WEBP to TIFF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <WebpToTiffToolCardIcon disableGroupHover />
              </div>
