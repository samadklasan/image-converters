...
import { BmpToWebpToolCardIcon } from "@/components/tools/bmp-to-webp/bmp-to-webp-card-icon";
...
function isValidBmpFile(file: File) {
  return isBmpToWebpMimeType(file.type) || isBmpFileName(file.name);
}
...
export function BmpToWebpConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > bmpToWebpMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${bmpToWebpMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getBmpToWebpDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/bmp-to-webp/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this BMP to WEBP batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <BmpToWebpToolCardIcon disableGroupHover />
              </div>
