...
import { WebpToJpgToolCardIcon } from "@/components/tools/webp-to-jpg/webp-to-jpg-card-icon";
import { createRuntimeId } from "@/lib/runtime-id";

const webpToJpgAcceptedMimeTypes = ["image/webp"] as const;
const webpToJpgMaxUploadBytes = 50 * 1024 * 1024;
const webpToJpgMaxUploadMegabytes = Math.floor(
  webpToJpgMaxUploadBytes / (1024 * 1024),
);
const webpToJpgMaxInputPixels = 60_000_000;
function isWebpToJpgMimeType(value: string) {
  return webpToJpgAcceptedMimeTypes.includes(
    value as (typeof webpToJpgAcceptedMimeTypes)[number],
  );
}
...
function getWebpToJpgDownloadName(value: string) {
...
const batchLimit = 20;
const queueLimitMessage = `You can convert up to ${batchLimit} WEBP images at a time.`;
...
function isValidWebpFile(file: File) {
  return isWebpToJpgMimeType(file.type) || isWebpFileName(file.name);
}
...
export function WebpToJpgConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        if (!isValidWebpFile(file)) {
          nextErrors.push(`"${file.name}" is not a valid WEBP image.`);
          continue;
        }

        if (file.size > webpToJpgMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${webpToJpgMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getWebpToJpgDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await readApiErrorMessage(
            response,
            "Unable to convert this WEBP image right now.",
          ),
        );
      }
      ...
      if (convertedBlob.size <= 0) {
        throw new Error("The converted JPG file was empty.");
      }
      ...
      throw conversionError instanceof Error
        ? conversionError
        : new Error("Unable to convert this WEBP image right now.");
    } finally {
  ...
  const convertQueue = async () => {
    ...
    if (itemIdsToConvert.length === 0) {
      setError("Select at least one WEBP image first.");
      return;
    }
    ...
    try {
      ...
      setError("Unable to finish this WEBP to JPG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
            ...
            <div className="ui-alert ui-alert-error font-body text-sm">
              ...
              <div className="mt-2 border-t border-red-300/80 pt-2">
                <p>
                  ...
                  you have more than 20 WEBP files, save this batch first, click
