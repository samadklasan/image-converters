...
import { JpgToBmpToolCardIcon } from "@/components/tools/jpg-to-bmp/jpg-to-bmp-card-icon";
...
const batchLimit = 20;
const queueLimitMessage = `You can convert up to ${batchLimit} JPG images at a time.`;
...
function isValidJpgFile(file: File) {
  return isJpgToBmpMimeType(file.type) || isJpgFileName(file.name);
}
...
export function JpgToBmpConverter() {
  ...
  const getConversionWorker = () => {
    ...
    const nextWorker = new Worker(
      new URL("./jpg-to-bmp-worker.ts", import.meta.url),
      { type: "module" },
    );
    ...
    nextWorker.onerror = (event) => {
      resetConversionWorker(
        event.message || "Unable to convert this JPG image right now.",
      );
    };
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        if (!isValidJpgFile(file)) {
          nextErrors.push(`"${file.name}" is not a JPG image.`);
          continue;
        }

        if (file.size > jpgToBmpMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${jpgToBmpMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getJpgToBmpDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertQueue = async () => {
    ...
    if (itemIdsToConvert.length === 0) {
      setError("Select at least one JPG image first.");
      return;
    }
    ...
    try {
      ...
      setError("Unable to finish this JPG to BMP batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
            ...
            <div className="ui-alert ui-alert-error font-body text-sm">
              ...
              <div className="mt-2 border-t border-red-300/80 pt-2">
                <p>
                  ...
                  you have more than 20 JPG files, save this batch first, click
