...
import { AvifToBmpToolCardIcon } from "@/components/tools/avif-to-bmp/avif-to-bmp-card-icon";
...
function isValidAvifFile(file: File) {
  return isAvifToBmpMimeType(file.type) || isAvifFileName(file.name);
}
...
export function AvifToBmpConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > avifToBmpMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${avifToBmpMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getAvifToBmpDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/avif-to-bmp/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this AVIF to BMP batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <AvifToBmpToolCardIcon disableGroupHover />
              </div>
