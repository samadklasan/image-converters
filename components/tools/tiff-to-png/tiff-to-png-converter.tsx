...
import { TiffToPngToolCardIcon } from "@/components/tools/tiff-to-png/tiff-to-png-card-icon";
...
function isValidTiffFile(file: File) {
  return isTiffToPngMimeType(file.type) || isTiffFileName(file.name);
}
...
export function TiffToPngConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > tiffToPngMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${tiffToPngMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getTiffToPngDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/tiff-to-png/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this TIFF to PNG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <TiffToPngToolCardIcon disableGroupHover />
              </div>
