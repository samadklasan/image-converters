...
import { TiffToAvifToolCardIcon } from "@/components/tools/tiff-to-avif/tiff-to-avif-card-icon";
import { createRuntimeId } from "@/lib/runtime-id";

const tiffToAvifAcceptedMimeTypes = ["image/tiff"] as const;
const tiffToAvifMaxUploadBytes = 50 * 1024 * 1024;
const tiffToAvifMaxUploadMegabytes = Math.floor(
  tiffToAvifMaxUploadBytes / (1024 * 1024),
);
const tiffToAvifMaxInputPixels = 60_000_000;
function isTiffToAvifMimeType(value: string) {
  return tiffToAvifAcceptedMimeTypes.includes(
    value as (typeof tiffToAvifAcceptedMimeTypes)[number],
  );
}
...
function getTiffToAvifDownloadName(value: string) {
...
function isValidTiffFile(file: File) {
  return isTiffToAvifMimeType(file.type) || isTiffFileName(file.name);
}
...
function getZipDownloadName() {
  return "tiff-to-avif-images.zip";
}
...
export function TiffToAvifConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > tiffToAvifMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${tiffToAvifMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getTiffToAvifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/tiff-to-avif/", {
        body: formData,
        method: "POST",
      ...
      if (convertedBlob.size <= 0) {
        throw new Error("The converted AVIF file was empty.");
      }
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this TIFF to AVIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <TiffToAvifToolCardIcon disableGroupHover />
              </div>
