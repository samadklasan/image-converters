...
import { PngToGifToolCardIcon } from "@/components/tools/png-to-gif/png-to-gif-card-icon";
...
function isValidPngFile(file: File) {
  return isPngToGifMimeType(file.type) || isPngFileName(file.name);
}
...
export function PngToGifConverter() {
  ...
  const getConversionWorker = () => {
    ...
    const nextWorker = new Worker(
      new URL("./png-to-gif-worker.ts", import.meta.url),
      { type: "module" },
    );
    ...
    nextWorker.onmessage = (event: MessageEvent<ConversionWorkerResponse>) => {
      ...
      if (event.data.type === "success") {
        pendingRequest.resolve(
          new Blob([event.data.buffer], { type: "image/gif" }),
        );
        return;
      }
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > pngToGifMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${pngToGifMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getPngToGifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
    ...
    try {
      for (const itemId of itemIdsToConvert) {
        ...
        try {
          ...
          const gifBlob = await convertItem(latestItem);
          const nextResultUrl = registerObjectUrl(gifBlob);
      ...
      setError("Unable to finish this PNG to GIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <PngToGifToolCardIcon disableGroupHover />
              </div>
