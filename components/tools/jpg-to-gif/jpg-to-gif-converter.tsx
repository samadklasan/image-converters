...
import { JpgToGifToolCardIcon } from "@/components/tools/jpg-to-gif/jpg-to-gif-card-icon";
...
function isValidJpgFile(file: File) {
  return isJpgToGifMimeType(file.type) || isJpgFileName(file.name);
}
...
export function JpgToGifConverter() {
  ...
  const getConversionWorker = () => {
    ...
    const nextWorker = new Worker(
      new URL("./jpg-to-gif-worker.ts", import.meta.url),
      { type: "module" },
    );
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > jpgToGifMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${jpgToGifMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getJpgToGifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this JPG to GIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <JpgToGifToolCardIcon disableGroupHover />
              </div>
