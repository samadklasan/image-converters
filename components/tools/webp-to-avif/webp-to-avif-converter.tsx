...
import { WebpToAvifToolCardIcon } from "@/components/tools/webp-to-avif/webp-to-avif-card-icon";
...
function isValidWebpFile(file: File) {
  return isWebpToAvifMimeType(file.type) || isWebpFileName(file.name);
}
...
function getZipDownloadName() {
  return "webp-to-avif-images.zip";
}
...
export function WebpToAvifConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > webpToAvifMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${webpToAvifMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getWebpToAvifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this WEBP to AVIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <WebpToAvifToolCardIcon disableGroupHover />
              </div>
