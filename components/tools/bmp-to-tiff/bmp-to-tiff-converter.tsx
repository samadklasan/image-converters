...
import { BmpToTiffToolCardIcon } from "@/components/tools/bmp-to-tiff/bmp-to-tiff-card-icon";
...
const batchLimit = 20;
const queueLimitMessage = `You can convert up to ${batchLimit} BMP images at a time.`;
...
function isValidBmpFile(file: File) {
  return isBmpToTiffMimeType(file.type) || isBmpFileName(file.name);
}
...
export function BmpToTiffConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        if (!isValidBmpFile(file)) {
          nextErrors.push(`"${file.name}" is not a valid BMP image.`);
          continue;
        }

        if (file.size > bmpToTiffMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${bmpToTiffMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getBmpToTiffDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/bmp-to-tiff/", {
        body: formData,
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(
          await readApiErrorMessage(
            response,
            "Unable to convert this BMP image right now.",
          ),
        );
      }
      ...
      throw conversionError instanceof Error
        ? conversionError
        : new Error("Unable to convert this BMP image right now.");
    } finally {
  ...
  const convertQueue = async () => {
    ...
    if (itemIdsToConvert.length === 0) {
      setError("Select at least one BMP image first.");
      return;
    }
    ...
    try {
      ...
      setError("Unable to finish this BMP to TIFF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
      ...
      <div className="space-y-4 px-3 py-3 sm:px-4 sm:py-4">
        ...
        >
          ...
          <input
            ref={fileInputRef}
            type="file"
            accept="image/bmp,image/x-ms-bmp,.bmp"
            multiple
            ...
            <div className="ui-alert ui-alert-error font-body text-sm">
              ...
              <div className="mt-2 border-t border-red-300/80 pt-2">
                <p>
                  ...
                  you have more than 20 BMP files, save this batch first, click
