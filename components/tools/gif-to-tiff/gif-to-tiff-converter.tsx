...
import { GifToTiffToolCardIcon } from "@/components/tools/gif-to-tiff/gif-to-tiff-card-icon";
...
function isValidGifFile(file: File) {
  return isGifToTiffMimeType(file.type) || isGifFileName(file.name);
}
...
export function GifToTiffConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > gifToTiffMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${gifToTiffMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getGifToTiffDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/gif-to-tiff/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this GIF to TIFF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <GifToTiffToolCardIcon disableGroupHover />
              </div>
