...
import { GifToJpgToolCardIcon } from "@/components/tools/gif-to-jpg/gif-to-jpg-card-icon";
...
function isValidGifFile(file: File) {
  return isGifToJpgMimeType(file.type) || isGifFileName(file.name);
}
...
export function GifToJpgConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > gifToJpgMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${gifToJpgMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getGifToJpgDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/gif-to-jpg/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this GIF to JPG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <GifToJpgToolCardIcon disableGroupHover />
              </div>
