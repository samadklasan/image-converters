...
import { PngToWebpToolCardIcon } from "@/components/tools/png-to-webp/png-to-webp-card-icon";
...
function isValidPngFile(file: File) {
  return isPngToWebpMimeType(file.type) || isPngFileName(file.name);
}
...
export function PngToWebpConverter() {
  ...
  const getConversionWorker = () => {
    ...
    const nextWorker = new Worker(
      new URL("./png-to-webp-worker.ts", import.meta.url),
      { type: "module" },
    );
    ...
    nextWorker.onmessage = (event: MessageEvent<ConversionWorkerResponse>) => {
      ...
      if (event.data.type === "success") {
        pendingRequest.resolve(
          new Blob([event.data.buffer], { type: "image/webp" }),
        );
        return;
      }
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > pngToWebpMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${pngToWebpMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getPngToWebpDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
    ...
    try {
      for (const itemId of itemIdsToConvert) {
        ...
        try {
          ...
          const webpBlob = await convertItem(latestItem);
          const nextResultUrl = registerObjectUrl(webpBlob);
      ...
      setError("Unable to finish this PNG to WEBP batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <PngToWebpToolCardIcon disableGroupHover />
              </div>
