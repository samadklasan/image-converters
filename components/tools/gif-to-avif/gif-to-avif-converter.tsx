...
import { GifToAvifToolCardIcon } from "@/components/tools/gif-to-avif/gif-to-avif-card-icon";
import { createRuntimeId } from "@/lib/runtime-id";

const gifToAvifAcceptedMimeTypes = ["image/gif"] as const;
const gifToAvifMaxUploadBytes = 50 * 1024 * 1024;
const gifToAvifMaxUploadMegabytes = Math.floor(
  gifToAvifMaxUploadBytes / (1024 * 1024),
);
const gifToAvifMaxInputPixels = 60_000_000;
function isGifToAvifMimeType(value: string) {
  return gifToAvifAcceptedMimeTypes.includes(
    value as (typeof gifToAvifAcceptedMimeTypes)[number],
  );
}
...
function getGifToAvifDownloadName(value: string) {
...
const batchLimit = 20;
const queueLimitMessage = `You can convert up to ${batchLimit} GIF images at a time.`;
...
function isValidGifFile(file: File) {
  return isGifToAvifMimeType(file.type) || isGifFileName(file.name);
}
...
export function GifToAvifConverter() {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        if (!isValidGifFile(file)) {
          nextErrors.push(`"${file.name}" is not a valid GIF image.`);
          continue;
        }

        if (file.size > gifToAvifMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${gifToAvifMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getGifToAvifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await readApiErrorMessage(
            response,
            "Unable to convert this GIF image right now.",
          ),
        );
      }
      ...
      throw conversionError instanceof Error
        ? conversionError
        : new Error("Unable to convert this GIF image right now.");
    } finally {
  ...
  const convertQueue = async () => {
    ...
    if (itemIdsToConvert.length === 0) {
      setError("Select at least one GIF image first.");
      return;
    }
    ...
    try {
      ...
      setError("Unable to finish this GIF to AVIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
            ...
            <div className="ui-alert ui-alert-error font-body text-sm">
              ...
              <div className="mt-2 border-t border-red-300/80 pt-2">
                <p>
                  ...
                  you have more than 20 GIF files, save this batch first, click
