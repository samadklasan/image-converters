...
import { AvifToTiffToolCardIcon } from "@/components/tools/avif-to-tiff/avif-to-tiff-card-icon";
...
function isValidAvifFile(file: File) {
  return isAvifToTiffMimeType(file.type) || isAvifFileName(file.name);
}
...
function getZipDownloadName() {
  return "avif-to-tiff-images.zip";
}
...
export function AvifToTiffConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > avifToTiffMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${avifToTiffMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getAvifToTiffDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/avif-to-tiff/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this AVIF to TIFF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <AvifToTiffToolCardIcon disableGroupHover />
              </div>
