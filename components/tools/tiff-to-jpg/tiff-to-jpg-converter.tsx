...
import { TiffToJpgToolCardIcon } from "@/components/tools/tiff-to-jpg/tiff-to-jpg-card-icon";
...
function isValidTiffFile(file: File) {
  return isTiffToJpgMimeType(file.type) || isTiffFileName(file.name);
}
...
export function TiffToJpgConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > tiffToJpgMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${tiffToJpgMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getTiffToJpgDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/tiff-to-jpg/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this TIFF to JPG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <TiffToJpgToolCardIcon disableGroupHover />
              </div>
