...
import { WebpToBmpToolCardIcon } from "@/components/tools/webp-to-bmp/webp-to-bmp-card-icon";
...
function isValidWebpFile(file: File) {
  return isWebpToBmpMimeType(file.type) || isWebpFileName(file.name);
}
...
export function WebpToBmpConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > webpToBmpMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${webpToBmpMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getWebpToBmpDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/webp-to-bmp/", {
        body: formData,
        method: "POST",
      ...
      if (convertedBlob.size <= 0) {
        throw new Error("The converted BMP file was empty.");
      }
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this WEBP to BMP batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <WebpToBmpToolCardIcon disableGroupHover />
              </div>
