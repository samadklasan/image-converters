...
import { BmpToPngToolCardIcon } from "@/components/tools/bmp-to-png/bmp-to-png-card-icon";
...
function isValidBmpFile(file: File) {
  return isBmpToPngMimeType(file.type) || isBmpFileName(file.name);
}
...
export function BmpToPngConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > bmpToPngMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${bmpToPngMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getBmpToPngDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/bmp-to-png/", {
        body: formData,
        method: "POST",
      ...
      if (convertedBlob.size <= 0) {
        throw new Error("The converted PNG file was empty.");
      }
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this BMP to PNG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <BmpToPngToolCardIcon disableGroupHover />
              </div>
