...
import { BmpToAvifToolCardIcon } from "@/components/tools/bmp-to-avif/bmp-to-avif-card-icon";
...
function isValidBmpFile(file: File) {
  return isBmpToAvifMimeType(file.type) || isBmpFileName(file.name);
}
...
export function BmpToAvifConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > bmpToAvifMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${bmpToAvifMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getBmpToAvifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/bmp-to-avif/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this BMP to AVIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <BmpToAvifToolCardIcon disableGroupHover />
              </div>
