...
import { PngToAvifToolCardIcon } from "@/components/tools/png-to-avif/png-to-avif-card-icon";
...
function isValidPngFile(file: File) {
  return isPngToAvifMimeType(file.type) || isPngFileName(file.name);
}
...
export function PngToAvifConverter() {
  ...
  const getConversionWorker = () => {
    ...
    const nextWorker = new Worker(
      new URL("./png-to-avif-worker.ts", import.meta.url),
      { type: "module" },
    );
    ...
    nextWorker.onmessage = (event: MessageEvent<ConversionWorkerResponse>) => {
      ...
      if (event.data.type === "success") {
        pendingRequest.resolve(
          new Blob([event.data.buffer], { type: "image/avif" }),
        );
        return;
      }
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > pngToAvifMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${pngToAvifMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getPngToAvifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
    ...
    try {
      for (const itemId of itemIdsToConvert) {
        ...
        try {
          ...
          const avifBlob = await convertItem(latestItem);
          const nextResultUrl = registerObjectUrl(avifBlob);
      ...
      setError("Unable to finish this PNG to AVIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <PngToAvifToolCardIcon disableGroupHover />
              </div>
