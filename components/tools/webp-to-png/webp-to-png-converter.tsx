...
import { WebpToPngToolCardIcon } from "@/components/tools/webp-to-png/webp-to-png-card-icon";
...
function isValidWebpFile(file: File) {
  return isWebpToPngMimeType(file.type) || isWebpFileName(file.name);
}
...
export function WebpToPngConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > webpToPngMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${webpToPngMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getWebpToPngDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this WEBP to PNG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <WebpToPngToolCardIcon disableGroupHover />
              </div>
