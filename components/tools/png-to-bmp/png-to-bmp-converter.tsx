...
import { PngToBmpToolCardIcon } from "@/components/tools/png-to-bmp/png-to-bmp-card-icon";
...
function isValidPngFile(file: File) {
  return isPngToBmpMimeType(file.type) || isPngFileName(file.name);
}
...
export function PngToBmpConverter() {
  ...
  const getConversionWorker = () => {
    ...
    const nextWorker = new Worker(
      new URL("./png-to-bmp-worker.ts", import.meta.url),
      { type: "module" },
    );
    ...
    nextWorker.onmessage = (event: MessageEvent<ConversionWorkerResponse>) => {
      ...
      if (event.data.type === "success") {
        pendingRequest.resolve(
          new Blob([event.data.buffer], { type: "image/bmp" }),
        );
        return;
      }
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > pngToBmpMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${pngToBmpMaxUploadMegabytes} MB.`,
          );
          continue;
        }

        acceptedItems.push({
          downloadFileName: getPngToBmpDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
    ...
    try {
      for (const itemId of itemIdsToConvert) {
        ...
        try {
          ...
          const bmpBlob = await convertItem(latestItem);
          const nextResultUrl = registerObjectUrl(bmpBlob);
      ...
      setError("Unable to finish this PNG to BMP batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <PngToBmpToolCardIcon disableGroupHover />
              </div>
            ...
            <div className="flex flex-col items-center justify-center gap-3 sm:flex-row sm:flex-wrap">
              <div className="group relative inline-flex">
                <button
                  ...
                  className="btn-primary h-9 min-w-44 gap-2 px-5"
                >
                  {isConverting ? "Converting..." : "Convert to BMP"}
                </button>
