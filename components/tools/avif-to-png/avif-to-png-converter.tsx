...
import { AvifToPngToolCardIcon } from "@/components/tools/avif-to-png/avif-to-png-card-icon";
import { createRuntimeId } from "@/lib/runtime-id";

const avifToPngAcceptedMimeTypes = ["image/avif"] as const;
const avifToPngMaxUploadBytes = 50 * 1024 * 1024;
const avifToPngMaxUploadMegabytes = Math.floor(
  avifToPngMaxUploadBytes / (1024 * 1024),
);
const avifToPngMaxInputPixels = 60_000_000;
function isAvifToPngMimeType(value: string) {
  return avifToPngAcceptedMimeTypes.includes(
    value as (typeof avifToPngAcceptedMimeTypes)[number],
  );
}
...
function getAvifToPngDownloadName(value: string) {
...
function isValidAvifFile(file: File) {
  return isAvifToPngMimeType(file.type) || isAvifFileName(file.name);
}
...
export function AvifToPngConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > avifToPngMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${avifToPngMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getAvifToPngDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/avif-to-png/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this AVIF to PNG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <AvifToPngToolCardIcon disableGroupHover />
              </div>
