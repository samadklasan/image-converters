...
import { TiffToGifToolCardIcon } from "@/components/tools/tiff-to-gif/tiff-to-gif-card-icon";
...
function isValidTiffFile(file: File) {
  return isTiffToGifMimeType(file.type) || isTiffFileName(file.name);
}
...
export function TiffToGifConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > tiffToGifMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${tiffToGifMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getTiffToGifDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertItem = async (item: QueueItem) => {
    ...
    try {
      ...
      const response = await fetch("/api/tiff-to-gif/", {
        body: formData,
        method: "POST",
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this TIFF to GIF batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <TiffToGifToolCardIcon disableGroupHover />
              </div>
