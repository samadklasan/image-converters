...
import { PngToJpgToolCardIcon } from "@/components/tools/png-to-jpg/png-to-jpg-card-icon";
...
const batchLimit = 20;
const queueLimitMessage = `You can convert up to ${batchLimit} PNG images at a time.`;
...
function isValidPngFile(file: File) {
  return isPngToJpgMimeType(file.type) || isPngFileName(file.name);
}
...
export function PngToJpgConverter() {
  ...
  const activeWorkerItemIdRef = useRef<string | null>(null);
  const pendingWorkerRequestRef = useRef<{
  ...
  useEffect(() => {
    return () => {
      ...
      if (pendingRequest) {
        pendingRequest.reject(new Error("Conversion cancelled."));
  ...
  const resetConversionWorker = (errorMessage = "Conversion cancelled.") => {
    disposeConversionWorker();
    ...
    activeWorkerItemIdRef.current = null;
  };
  ...
  const getConversionWorker = () => {
    ...
    const nextWorker = new Worker(
      new URL("./png-to-jpg-worker.ts", import.meta.url),
      { type: "module" },
    );

    nextWorker.onmessage = (event: MessageEvent<ConversionWorkerResponse>) => {
      const pendingRequest = pendingWorkerRequestRef.current;
      ...
      if (event.data.type === "success") {
        pendingRequest.resolve(
          new Blob([event.data.buffer], { type: "image/jpeg" }),
        );
        return;
      }
    ...
    nextWorker.onerror = (event) => {
      resetConversionWorker(
        event.message || "Unable to convert this PNG image right now.",
      );
    };
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        if (!isValidPngFile(file)) {
          nextErrors.push(`"${file.name}" is not a PNG image.`);
          continue;
        }

        if (file.size > pngToJpgMaxUploadBytes) {
          nextErrors.push(
            `"${file.name}" is larger than ${pngToJpgMaxUploadMegabytes} MB.`,
          );
          continue;
        }
        ...
        acceptedItems.push({
          id: createRuntimeId(),
          downloadFileName: getPngToJpgDownloadName(file.name),
          error: "",
          file,
  ...
  const removeQueueItem = (itemId: string) => {
    if (activeWorkerItemIdRef.current === itemId) {
      resetConversionWorker();
    }
  ...
  const convertItem = async (item: QueueItem) => {
    return new Promise<Blob>((resolve, reject) => {
      ...
      activeWorkerItemIdRef.current = item.id;
      pendingWorkerRequestRef.current = {
        reject,
        requestId,
        resolve,
      };

      getConversionWorker().postMessage({
        file: item.file,
        requestId,
        type: "convert",
      });
  ...
  const convertQueue = async () => {
    ...
    if (itemIdsToConvert.length === 0) {
      setError("Select at least one PNG image first.");
      return;
    }
    ...
    try {
      for (const itemId of itemIdsToConvert) {
        ...
        try {
          ...
          const jpgBlob = await convertItem(latestItem);
          const nextResultUrl = registerObjectUrl(jpgBlob);
      ...
      setError("Unable to finish this PNG to JPG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
  ...
  const saveAll = async () => {
    const convertedItems = queue.filter(
      (item) => item.resultUrl && item.resultBlob,
    );
    ...
    try {
      ...
      const zipUrl = registerObjectUrl(
  ...
  const isConvertDisabled = !canConvert;
  const isSaveAllDisabled = !canSaveAll;
              ...
              <div className="grid grid-cols-[auto,minmax(0,1fr),auto] items-center gap-3 sm:gap-4">
                ...
                >
                  {queue.map((item, index) => {
                    ...
                    const previewSource = item.previewUrl;
                        ...
                        >
                          ...
                          {isProcessing ? (
                            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/45 p-4 text-center text-white">
                              <div className="h-8 w-8 rounded-full border-2 border-white/35 border-t-white animate-spin" />
                        ...
                        <div className="space-y-2.5 p-2.5">
                          ...
                          {isDone ? (
                            <button
                              type="button"
                              onClick={() =>
                                triggerDownload(item.resultUrl, item.downloadFileName)
                              }
            ...
            <div className="ui-alert ui-alert-error font-body text-sm">
              ...
              <div className="mt-2 border-t border-red-300/80 pt-2">
                <p>
                  ...
                  you have more than 20 PNG files, save this batch first, click
