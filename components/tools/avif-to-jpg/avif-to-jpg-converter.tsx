...
import { AvifToJpgToolCardIcon } from "@/components/tools/avif-to-jpg/avif-to-jpg-card-icon";
import { createRuntimeId } from "@/lib/runtime-id";

const avifToJpgAcceptedMimeTypes = ["image/avif"] as const;
const avifToJpgMaxUploadBytes = 50 * 1024 * 1024;
const avifToJpgMaxUploadMegabytes = Math.floor(
  avifToJpgMaxUploadBytes / (1024 * 1024),
);
const avifToJpgMaxInputPixels = 60_000_000;
function isAvifToJpgMimeType(value: string) {
  return avifToJpgAcceptedMimeTypes.includes(
    value as (typeof avifToJpgAcceptedMimeTypes)[number],
  );
}
...
function getAvifToJpgDownloadName(value: string) {
...
function isValidAvifFile(file: File) {
  return isAvifToJpgMimeType(file.type) || isAvifFileName(file.name);
}
...
export function AvifToJpgConverter() {
  ...
  const appendFiles = (fileList: FileList | File[]) => {
    ...
    setQueue((current) => {
      ...
      for (const file of nextFiles.slice(0, availableSlots)) {
        ...
        if (file.size > avifToJpgMaxUploadBytes) {
          nextErrors.push(`"${file.name}" is larger than ${avifToJpgMaxUploadMegabytes} MB.`);
          continue;
        }

        acceptedItems.push({
          downloadFileName: getAvifToJpgDownloadName(file.name),
          error: "",
          file,
          id: createRuntimeId(),
  ...
  const convertQueue = async () => {
    ...
    try {
      ...
      setError("Unable to finish this AVIF to JPG batch right now.");
    } finally {
      removedItemIdsRef.current.clear();
      setQueue((current) =>
        current.map((entry) =>
          ...
          {!hasQueue ? (
            <div className="flex min-h-[18rem] flex-col items-center justify-center gap-4 px-5 py-8 text-center sm:px-6 lg:px-8">
              <div className="text-brand">
                <AvifToJpgToolCardIcon disableGroupHover />
              </div>
