...
const gifToAvifOutputQuality = 60;
...
export async function POST(request: Request) {
  ...
  if (uploadedFile.size > gifToAvifMaxUploadBytes) {
    return jsonError("This GIF image is too large to convert safely right now.", 413);
  }

  if (
    !isGifToAvifMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: gifToAvifMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > gifToAvifMaxInputPixels
    ) {
      return jsonError(
    ...
    const convertedBuffer = await image
      .avif({
        quality: gifToAvifOutputQuality,
      })
      .toBuffer();
    ...
    return new Response(new Uint8Array(convertedBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getGifToAvifDownloadName(uploadedFile.name)}"`,
        "Content-Type": "image/avif",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    console.error("GIF to AVIF conversion failed", error);
    ...
    return jsonError("Unable to convert this GIF image to AVIF right now.", 500);
  }
}
