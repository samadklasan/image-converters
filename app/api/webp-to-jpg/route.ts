...
const webpToJpgOutputQuality = 85;
...
export async function POST(request: Request) {
  ...
  if (uploadedFile.size > webpToJpgMaxUploadBytes) {
    return jsonError("This WEBP image is too large to convert safely right now.", 413);
  }

  if (
    !isWebpToJpgMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: webpToJpgMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > webpToJpgMaxInputPixels
    ) {
      return jsonError(
    ...
    const convertedBuffer = await image
      ...
      .jpeg({
        mozjpeg: true,
        progressive: true,
        quality: webpToJpgOutputQuality,
      })
      .toBuffer();
    ...
    return new Response(new Uint8Array(convertedBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getWebpToJpgDownloadName(uploadedFile.name)}"`,
    ...
    return jsonError("Unable to convert this WEBP image to JPG right now.", 500);
  }
}
