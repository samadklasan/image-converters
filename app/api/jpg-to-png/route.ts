...
export async function POST(request: NextRequest) {
  ...
  if (uploadedFile.size > jpgToPngMaxUploadBytes) {
    return jsonError("This JPG is too large to convert safely right now.", 413);
  }

  if (
    !isJpgToPngMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: jpgToPngMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > jpgToPngMaxInputPixels
    ) {
      return jsonError(
    ...
    return new Response(new Uint8Array(convertedBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getJpgToPngDownloadName(uploadedFile.name)}"`,
    ...
    return jsonError("Unable to convert this JPG image to PNG right now.", 500);
  }
}
