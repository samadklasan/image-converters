...
const tiffToAvifOutputQuality = 60;
...
export async function POST(request: Request) {
  ...
  if (uploadedFile.size > tiffToAvifMaxUploadBytes) {
    return jsonError("This TIFF image is too large to convert safely right now.", 413);
  }

  if (
    !isTiffToAvifMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: tiffToAvifMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > tiffToAvifMaxInputPixels
    ) {
      return jsonError(
    ...
    const convertedBuffer = await image
      .avif({
        quality: tiffToAvifOutputQuality,
      })
      .toBuffer();
    ...
    return new Response(new Uint8Array(convertedBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getTiffToAvifDownloadName(uploadedFile.name)}"`,
        "Content-Type": "image/avif",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    console.error("TIFF to AVIF conversion failed", error);
    ...
    return jsonError("Unable to convert this TIFF image to AVIF right now.", 500);
  }
}
