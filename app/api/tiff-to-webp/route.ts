...
const tiffToWebpOutputQuality = 75;
...
export async function POST(request: Request) {
  ...
  if (uploadedFile.size > tiffToWebpMaxUploadBytes) {
    return jsonError("This TIFF image is too large to convert safely right now.", 413);
  }

  if (
    !isTiffToWebpMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: tiffToWebpMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > tiffToWebpMaxInputPixels
    ) {
      return jsonError(
    ...
    const convertedBuffer = await image
      .webp({
        quality: tiffToWebpOutputQuality,
      })
      .toBuffer();
    ...
    return new Response(new Uint8Array(convertedBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getTiffToWebpDownloadName(uploadedFile.name)}"`,
        "Content-Type": "image/webp",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    console.error("TIFF to WEBP conversion failed", error);
    ...
    return jsonError("Unable to convert this TIFF image to WEBP right now.", 500);
  }
}
