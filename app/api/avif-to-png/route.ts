...
export async function POST(request: Request) {
  ...
  if (uploadedFile.size > avifToPngMaxUploadBytes) {
    return jsonError("This AVIF image is too large to convert safely right now.", 413);
  }

  if (
    !isAvifToPngMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: avifToPngMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > avifToPngMaxInputPixels
    ) {
      return jsonError(
    ...
    return new Response(new Uint8Array(convertedBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getAvifToPngDownloadName(uploadedFile.name)}"`,
    ...
    return jsonError("Unable to convert this AVIF image to PNG right now.", 500);
  }
}
