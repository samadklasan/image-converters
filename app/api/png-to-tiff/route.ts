...
export async function POST(request: NextRequest) {
  ...
  if (uploadedFile.size > pngToTiffMaxUploadBytes) {
    return jsonError("This PNG is too large to convert safely right now.", 413);
  }

  if (
    !isPngToTiffMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: pngToTiffMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > pngToTiffMaxInputPixels
    ) {
      return jsonError(
    ...
    return new Response(new Uint8Array(convertedBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getPngToTiffDownloadName(uploadedFile.name)}"`,
        "Content-Type": "image/tiff",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    console.error("PNG to TIFF conversion failed", error);
    ...
    return jsonError("Unable to convert this PNG image to TIFF right now.", 500);
  }
}
