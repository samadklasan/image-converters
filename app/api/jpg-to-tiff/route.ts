...
export async function POST(request: NextRequest) {
  ...
  if (uploadedFile.size > jpgToTiffMaxUploadBytes) {
    return jsonError("This JPG is too large to convert safely right now.", 413);
  }

  if (
    !isJpgToTiffMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: jpgToTiffMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > jpgToTiffMaxInputPixels
    ) {
      return jsonError(
    ...
    return new Response(new Uint8Array(convertedBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getJpgToTiffDownloadName(uploadedFile.name)}"`,
        "Content-Type": "image/tiff",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    console.error("JPG to TIFF conversion failed", error);
    ...
    return jsonError("Unable to convert this JPG image to TIFF right now.", 500);
  }
}
