...
const avifToJpgOutputQuality = 85;
...
export async function POST(request: Request) {
  ...
  if (uploadedFile.size > avifToJpgMaxUploadBytes) {
    return jsonError("This AVIF image is too large to convert safely right now.", 413);
  }

  if (
    !isAvifToJpgMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: avifToJpgMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > avifToJpgMaxInputPixels
    ) {
      return jsonError(
    ...
    const convertedBuffer = await image
      ...
      .jpeg({
        mozjpeg: true,
        progressive: true,
        quality: avifToJpgOutputQuality,
      })
      .toBuffer();
    ...
    return new Response(new Uint8Array(convertedBuffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getAvifToJpgDownloadName(uploadedFile.name)}"`,
        "Content-Type": "image/jpeg",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    console.error("AVIF to JPG conversion failed", error);
    ...
    return jsonError("Unable to convert this AVIF image to JPG right now.", 500);
  }
}
