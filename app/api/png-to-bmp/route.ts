...
export async function POST(request: NextRequest) {
  ...
  if (uploadedFile.size > pngToBmpMaxUploadBytes) {
    return jsonError("This PNG is too large to convert safely right now.", 413);
  }

  if (
    !isPngToBmpMimeType(uploadedFile.type) &&
  ...
  try {
    const image = sharp(inputBuffer, {
      limitInputPixels: pngToBmpMaxInputPixels,
      sequentialRead: true,
    });
    ...
    if (
      ...
      metadata.width * metadata.height > pngToBmpMaxInputPixels
    ) {
      return jsonError(
    ...
    return new Response(convertedBuffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${getPngToBmpDownloadName(uploadedFile.name)}"`,
        "Content-Type": "image/bmp",
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    console.error("PNG to BMP conversion failed", error);
    ...
    return jsonError("Unable to convert this PNG image to BMP right now.", 500);
  }
}
